    ...
    <div className="grid grid-cols-2 gap-4 md:gap-6 pb-20">
      {options.map((option) => {
        if (option.status === 'generating_base') {
           return (
             <div key={option.id} className="aspect-square bg-slate-900/50 rounded-2xl animate-pulse ring-1 ring-slate-800 flex items-center justify-center">
                <div className="flex flex-col items-center space-y-2">
                   <svg className="animate-spin h-6 w-6 text-indigo-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    {option.prompt && (
                      <span className="text-[10px] text-slate-500 max-w-[80%] truncate text-center px-2">
                        {option.prompt}
                      </span>
                    )}
                </div>
